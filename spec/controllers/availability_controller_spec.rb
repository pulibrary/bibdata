...
RSpec.describe AvailabilityController, type: :controller do
  ...
  describe 'bib availability hash' do
    it 'provides availability for only the first 2 holdings by default' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("929437"=>{ "1068356"=>{ more_items: false, location: "rcppa", status: ["Not Charged"] }, "1068357"=>{ more_items: false, location: "fnc", status: ["Not Charged"] } })
    ...
    it 'provides availability for all holdings if full availability requested' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("1068356"=>{ more_items: false, location: "rcppa", status: ["Not Charged"] }, "1068357"=>{ more_items: false, location: "fnc", status: ["Not Charged"] }, "1068358"=>{ more_items: false, location: "anxb", status: ["Not Charged"] })
      bib_3_holdings = '929437'
  ...
  describe 'holding availability hash' do
    ...
    it 'voyager status is returned for item' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("35345"=>{ "39176"=>{ more_items: false, location: "f", status: ["Not Charged"] } })
      bib_with_item = '35345'
      holding_id = '39176'
      item_id = 36736
    ...
    context 'when a connection error is encountered for Voyager' do
      ...
      before do
        ...
        allow(Voyager::Adapter).to receive(:get_availability).and_raise(OCIError)
      end
    ...
    it 'elf records have a status of online' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("7916044"=>{ "7698138"=>{ more_items: false, location: "elf1", status: "Online" }, "7860428"=>{ more_items: false, location: "rcpph", status: ["Not Charged"] } })
      bib_online = '7916044'
      holding_id = '7698138'
    ...
    it 'on-order records have a status of On-Order' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("9173362"=>{ "9051785"=>{ more_items: false, location: "f", status: "On-Order 09-10-2015" } })
    ...
    it 'limited access location on-order records have a status of On-Order' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("9355531"=>{ more_items: false, location: "sa", status: "On-Order" })
      marquand = '9497429'
    ...
    it 'online on-order records have a status of On-Order' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("9096471"=>{ more_items: false, location: "elf1", status: "On-Order" })
      online = '9226664'
    ...
    it 'Received on-order records have a status of Order Received' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("9329199"=>{ more_items: false, location: "f", status: "Order Received 12-16-2015" })
    ...
    describe 'location-based availability' do
      it 'always_requestable locations should display order information status' do
        ...
        allow(Voyager::Adapter).to receive(:get_availability).and_return("4609321"=>{ "4847980"=>{ more_items: false, location: "whs", status: "Order Received 12-16-2015" } })
        bible = '4609321'
        holding_id = '4847980'
      ...
      it 'non_circulating, always_requestable locations have a status of on-site when available' do
        ...
        allow(Voyager::Adapter).to receive(:get_availability).and_return("4609321"=>{ "4847980"=>{ more_items: false, location: "whs", status: "On Shelf" } })
        bible = '4609321'
        holding_id = '4847980'
        get :index, params: { ids: [bible], format: :json }
        availability = JSON.parse(response.body)
      ...
      it 'non_circulating, always_requestable locations include status when unavailable' do
        ...
        allow(Voyager::Adapter).to receive(:get_availability).and_return("4609321"=>{ "4847980"=>{ more_items: false, location: "whs", status: "Unavailable" } })
        bible = '4609321'
        holding_id = '4847980'
        get :index, params: { ids: [bible], format: :json }
        availability = JSON.parse(response.body)
    ...
    it 'Items with temp location codes are have a temp_loc key' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("9329199"=>{ more_items: false, location: "f", temp_loc: temp_loc, status: ["Charged"] })
      bib_received_order = '9468468'
      get :index, params: { id: bib_received_order, format: :json }
      availability = JSON.parse(response.body)
    ...
    it 'Items with temp location codes are mapped the display value' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("9329199"=>{ more_items: false, location: "f", temp_loc: 'woooo', status: ["Charged"] })
      bib_received_order = '9468468'
      get :index, params: { id: bib_received_order, format: :json }
      availability = JSON.parse(response.body)
    ...
    it 'all other holding records without items have a status of On Shelf' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("7617477"=>{ "7429805"=>{ more_items: false, location: "f", status: "On Shelf" }, "7429809"=>{ more_items: false, location: "sci", status: "On Shelf" } })
      bib_ipad = '7617477'
      holding_id = '7429805'
    ...
    it 'more_items is true when there is more than 1 item for a holding' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return("857469"=>{ "977093"=>{ more_items: true, location: "f", status: ["Not Charged"] }, "977094"=>{ more_items: true, location: "rcppf", status: ["Not Charged"] } })
  ...
  describe 'availability for item with multiple statuses' do
    it 'returns status with highest priority' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return(7135944=>{ "7002641"=>{ more_items: false, location: "mus", copy_number: 1, item_id: 6406359, on_reserve: "N", status: ["Overdue", "Lost--System Applied", "In Process"] } } )
  ...
  describe 'availability for items with hold request status' do
    ...
    it 'recap non-aeon item returns status of hold request' do
      ...
      allow(Voyager::Adapter).to receive(:get_availability).and_return(recap_non_aeon)
      get :index, params: { ids: [bib_id], format: :json }
      availability = JSON.parse(response.body)
  ...
  describe 'full mfhd availability array' do
    it 'returns info for all items for a given mfhd' do
      ...
      allow(Voyager::Adapter).to receive(:get_full_mfhd_availability).and_return([{ barcode: "32101082329929", id: 502918, status: ["Not Charged"], enum: "1994" }, { barcode: "32101082329937", id: 502919, status: ["Not Charged"], enum: "1995" }, { barcode: "32101082329945", id: 502920, status: ["Not Charged"], enum: "1996" }, { barcode: "32101035390275", id: 502921, status: ["Not Charged"], enum: "1954-59" }, { barcode: "32101082329895", id: 1767770, status: ["Not Charged"], enum: "1964" }])
      holding_id = '464473'
  ...
  describe 'mfhd_serial location has current' do
    it '404 when no volumes found for given mfhd' do
      ...
      allow(Voyager::Adpater).to receive(:get_current_issues).and_return([])
    ...
    it 'current volumes for given_mfhd are parsed as an array' do
      ...
      allow(Voyager::Adapter).to receive(:get_current_issues).and_return(['v1', 'v2', 'v3'])
      get :index, params: { mfhd_serial: '12345678901', format: :json }
  ...
  it "404 when bibs are not provided" do
    ...
    allow(Voyager::Adapter).to receive(:get_availability).and_return({})
