...
RSpec.describe AlmaAdapter do
  ...
  describe "holding availability from items" do
    before do
      stub_alma_ids(ids: "99125379706706421", status: 200)
      ...
      stub_alma_library(library_code: "firestone", location_code: "stacks")
      stub_alma_ids(ids: "99125378873406421", status: 200)
    ...
    it "reports available when all items are available" do
      availability = adapter.get_availability_one(id: "99125379706706421", deep_check: true)
      holding = availability["99125379706706421"]["22897520080006421"]
      expect(holding[:status_label]).to eq "Available"
    end

    it "reports unavailable when all items are unavailable" do
      availability = adapter.get_availability_one(id: "99125378873406421", deep_check: true)
      holding = availability["99125378873406421"]["22897164770006421"]
      expect(holding[:status_label]).to eq "Not Available"
    end

    it "reports some items not available when there is a mix of statuses" do
      availability = adapter.get_availability_one(id: "99125379706706421", deep_check: true)
      holding = availability["99125379706706421"]["22897390520006421"]
      expect(holding[:status_label]).to eq "Some items not available"
    end
  end
