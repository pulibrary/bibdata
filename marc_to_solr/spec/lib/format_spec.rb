...
describe 'From format.rb' do
  ...
  describe '502 note' do
    ...
    let(:senior_thesis_whitespace) { MARC::Record.new_from_hash({ 'fields' => [whitespace_502] }) }
    ...
    it 'whitespace is ignored in classifying senior thesis' do
      senior_thesis_whitespace.leader = marc.leader
      fmt = Format.new(senior_thesis_whitespace).bib_format
      expect(Traject::TranslationMap.new("format").translate_array!(fmt)).to include 'Senior thesis'
    end
