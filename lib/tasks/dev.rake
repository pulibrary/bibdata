      ...
      task :update, [:solr_dir] => :environment do |_t, args|
        ...
        ["_rest_managed.json", "admin-extra.html", "elevate.xml",
         ...
         "stopwords_en.txt", "synonyms.txt"].each do |file|
           response = Faraday.get url_for_file(file, test_fix)
           File.open(File.join(solr_dir, "conf", file), "wb") { |f| f.write(response.body) } if response.success?
         end
      end

      def url_for_file(file, test_fix)
        if test_fix
          # Use the config files from the fq_syntax branch to test the new configuration
          return "https://raw.githubusercontent.com/pulibrary/pul_solr/fq_syntax/solr_configs/catalog-staging/conf/#{file}"
        end
