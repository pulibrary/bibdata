...
impl From<&EphemeraFolder> for SolrDocument {
    fn from(value: &EphemeraFolder) -> Self {
        SolrDocument::builder()
            ...
            .with_pub_created_display(Some(value.origin_place_publisher_date_created_combined()))
            .with_publisher_no_display(value.publisher.clone())
            .with_publisher_citation_display(value.publisher.clone())
            .with_title_display(value.title.first().map(|s| s.as_str()))
            .with_title_sort_key(value.first_sort_title().as_deref())
            .with_title_citation_display(value.title.first().map(|t| t.as_str()))
            .with_thumbnail_display(value.thumbnail_url().as_deref())
            .build()
    }
}
