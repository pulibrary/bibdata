...
pub fn embargo_text(lift_dates: Option<Vec<String>>, terms_dates: Option<Vec<String>>, doc_id: String) -> String {
...
mod tests {
    ...
    fn test_has_current_embargo() {
        assert!(has_current_embargo(None, Some(vec!["2100-07-13".to_owned()])));
        assert!(has_current_embargo(Some(vec!["2100-07-13".to_owned()]), None));
        assert!(has_current_embargo(Some(vec!["2100-07-13".to_owned()]), Some(vec!["2100-08-15".to_owned()])));

        assert!(!has_current_embargo(None, None));
        assert!(!has_current_embargo(None, Some(vec!["1880-01-01".to_owned()])));
        assert!(!has_current_embargo(None, Some(vec!["unparseable date".to_owned()])));
    }

    #[test]
    fn test_embargo_date() {
        assert_eq!(embargo_date(None, Some(vec!["2100-07-13".to_owned()])).unwrap(), "July 13, 2100");
        assert_eq!(embargo_date(Some(vec!["2100-07-13".to_owned()]), None).unwrap(), "July 13, 2100");
        assert_eq!(embargo_date(Some(vec!["2100-07-13".to_owned()]), Some(vec!["2100-08-15".to_owned()])).unwrap(), "July 13, 2100");
        assert_eq!(embargo_date(None, Some(vec!["1880-01-01".to_owned()])), Some("January 1, 1880".to_owned()));

        assert_eq!(embargo_date(None, None), None);
        assert_eq!(embargo_date(None, Some(vec!["unparseable date".to_owned()])), None);
    }
