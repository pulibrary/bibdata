...
class CampusAccess < ActiveRecord::Base
  ...
  class << self
    ...
    def load_access(xlsx_filename, trained_file: nil, header_rows: 4, trailer_rows: 4, additional_ids: [])
      employee_id_lookup = {}
      unique_users = load_users(xlsx_filename, header_rows:, trailer_rows:, employee_id_lookup:)
      unique_trained_users = load_users(trained_file, header_rows:, trailer_rows:, filter: :filter_learn, employee_id_lookup:)
      ...
      CampusAccess.transaction do
        ...
        create_ids(full_ids, employee_id_lookup:)
        create_ids(unique_trained_users - full_ids, category: const_get("TRAINED"), employee_id_lookup:)
      end
    end
    ...
    private
      ...
      def create_ids(id_list, category: const_get("FULL"), employee_id_lookup:)
        id_list.each do |user|
          create(uid: user, category:, employee_id: employee_id_lookup[user]&.to_s&.rjust(9, "0"))
        end
      end
