...
def update_locations
  begin
    ...
    maps_dir = File.join(Rails.root, 'config', 'translation_maps')
    File.open(File.join(maps_dir, 'location_display.rb'), 'w') { |file| PP.pp(libdisplay, file) }
    File.open(File.join(maps_dir, 'locations.rb'), 'w') { |file| PP.pp(longerdisplay, file) }
